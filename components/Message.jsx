...
import Prism from 'prismjs' //for the code part like html,css,js etc
...
const Message = ({role,content}) => {
    ...
    const copyMessage =()=>{
        ...
        toast.success('Messsage Copied to clipboard')
    }

  return (
    <div className='flex flex-col items-center w-full max-w-3xl text-sm'>
      <div className={`flex flex-col w-full mt-8 ${role === 'user'&& 'items-end'}` }>
        <div className={`group relative flex max-w-2xl py-3 rounded-xl  ${role === 'user'? 'bg-[#414158] px-5':'gap-3'} `}>
            ...
            </div>
            {
                ...
                :
                (   <>
                    <Image alt='' src={assets.logo_icon} className='h-9 w-9 p-1 border border-white/15 rounded-full' />
